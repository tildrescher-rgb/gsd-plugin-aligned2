...
A PostToolUse hook also writes a fresh checkpoint after any file-mutating tool call (Bash, Edit, Write, MultiEdit, NotebookEdit) — throttled to at most once per 60 seconds. This bridges Claude Code's *microcompact* path, which silently strips stale tool outputs without firing PreCompact: the periodic checkpoint keeps `HANDOFF.json` at most ~60s stale at any point during an active session, so resume after an unexpected session end reflects recent state.
